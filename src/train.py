...
def create_image_lists(image_dir: str, testing_percentage: int, validation_percentage: int) -> dict:
    """
    Brief:
...
def create_bottleneck_file(bottleneck_path, image_lists: dict, label_name: str, index: int,
                           image_dir: str, category, sess, jpeg_data_tensor,
                           bottleneck_tensor):
...
def get_or_create_bottleneck(sess, image_lists: dict, label_name: str, index: int, image_dir: str,
                             category: str, bottleneck_dir: str, jpeg_data_tensor,
                             bottleneck_tensor) -> list[float]:
    """
    Brief:
...
def add_input_distortions(flip_left_right: bool, random_crop: int, random_scale: int,
  ...
  """
    ...
    resize_scale_value = tf.random.uniform(tensor_shape.TensorShape([]),
                                         minval=1,
                                         maxval=resize_scale)
    ...
    brightness_min = int(1.0 - (random_brightness / 100.0))
    brightness_max = int(1.0 + (random_brightness / 100.0))
    brightness_value = tf.random.uniform(tensor_shape.TensorShape([]),
